  ...
  return (
    <div className="relative flex justify-center items-center bg-white rounded-3xl p-4 border border-slate-100 shadow-sm overflow-visible min-h-[650px] w-full">
      <div className="relative w-full max-w-[600px] aspect-square">
        ...
        {data.map((d, i) => {
          // Calculate position using percentages so it scales with the container
          const distance = radius + 20;
          const xPercent = 50 + (distance / (width / 2)) * 50 * Math.cos(angleSlice * i - Math.PI / 2);
          const yPercent = 50 + (distance / (height / 2)) * 50 * Math.sin(angleSlice * i - Math.PI / 2);
          ...
          // Align labels radiating outwards from the points
          if (i === 0) {
            translateY = '-100%'; // Top
          } else if (i === 3) {
            translateY = '0%'; // Bottom
          } else if (i === 1 || i === 2) {
            translateX = '0%'; // Right side
          } else if (i === 4 || i === 5) {
          ...
          return (
            <div
              key={d.name}
              className="absolute flex items-center gap-2 p-1.5 bg-white/95 backdrop-blur-sm rounded-xl border border-slate-100 shadow-sm transition-all hover:shadow-md hover:scale-105 z-10"
              style={{
                left: `${xPercent}%`,
                top: `${yPercent}%`,
            ...
            >
              ...
              <div className="flex flex-col min-w-0 pr-2">
                ...
                <span className="text-[10px] font-bold text-slate-500 leading-tight line-clamp-2 whitespace-normal">
                  {d.description}
                </span>
              </div>
            </div>
