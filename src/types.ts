...
export interface IntakeData {
  // 第 0 步：入口分流
  respondentType: 'self' | 'family' | 'other';
  currentStage: 'hospitalized' | 'just-discharged' | 'home-awhile' | 'stable' | 'planning-work';
  primaryConcern: 'care' | 'daily-life' | 'cognition' | 'work-economy' | 'family-pressure' | 'social-emotional' | 'housing-transport' | 'unknown';
  ...
  // 4. Work & Economy
  considerWork?: 'yes' | 'maybe' | 'no' | 'later';
  economy: {
    skills: number;
    intent: number;
    capacity: number;
...
export interface RiskItem {
  type: 'fall' | 'burnout' | 'isolation' | 'institutionalization' | 'economic' | 'other';
